...
-- Dumping structure for procedure hplydb_new.proc_calc_interest_by_project
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_calc_interest_by_project`(IN `in_project_id` vaRCHAR(50))
    COMMENT '计算指定工程的里'
BEGIN
	declare _offset_amount decimal(20, 4) default 0.0;
	
	-- 计算最后一次计息基数，若没有，则为0，计息数据version=9001
	select offset_amount into _offset_amount from t_arrears where serial_id=(select max(serial_id) from t_arrears where project_id=in_project_id and version=9001);
