...
set arg0=upper(arg0);
select param_value into next_id from t_sys_parameter where upper(en_name)=arg0 and EXTRACT(YEAR FROM update_time) = EXTRACT(YEAR FROM now());
...
set next_code = concat(arg0, '-', EXTRACT(YEAR FROM now()), '-', lpad(next_id, 4, '0'));
