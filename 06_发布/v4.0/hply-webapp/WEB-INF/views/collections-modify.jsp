...
<link rel="stylesheet" href="<s:url value="/assets/selectize/css/selectize.bootstrap3.css" />">
<script src="<s:url value="/assets/selectize/js/standalone/selectize.min.js" />"></script>
                    ...
                    <div class="col-sm-8">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-4 ">
                                ...
                                <sf:select cssClass="form-control" path="field01" id="field01"/>
                                <p class="help-block" />
                            </div>
                        ...
                        <a href='<s:url value="/collections" />' onclick="return goback();" class="btn btn-link"><span
...
<script type="text/javascript">
	$(function() {
		var field01 = "${collections.field01}";
		var $field01 = $("select[id=field01]").selectize({
			valueField: 'id',
			labelField: 'title',
			searchField: 'title',
			createOnBlur: true,
    	...
    	var field01Control = $field01[0].selectize;
    	field01Control.setValue("${collections.field01}");
		$("#projectId").change(function(){
			refreshAmount($(this).val());
			$.ajax({
                url:"<s:url value='/partybilling/getorglist.json' />",
                type:'get',
                async: false,
                data:{projectId: $(this).val()},
                success: function(res) {
                	// 重新生成option之前先清除前面生成的option
                	field01Control.clearOptions();
                	var isExists = false;
                    if(res){
                    	for(var i = 0; i < res.length; i++){
                    		var orgName = res[i]["orgName"];
                    		if(field01 && field01 == orgName){
                    			isExists = true;
                    		}
                    		field01Control.addOption({
                                id : orgName,
                                title : orgName,
                            });
                        }
        ...
        field01Control.setValue(field01);
	});
</script>
