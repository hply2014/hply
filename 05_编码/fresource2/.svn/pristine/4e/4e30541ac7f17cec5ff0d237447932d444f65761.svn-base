...
public class FieldTypesController {
	...
	public String list(Model model) {
		model.addAttribute("page_title", "一个Demo测试样表");
	...
	public String detail(@PathVariable String id, Model model) {
		model.addAttribute("page_title", "一个Demo测试样表的详情信息");
	...
	public String createForm(Model model) {
		...
		model.addAttribute("page_title", "新建一个Demo测试样表");
		String[] types = new String[]{"类型一", "类型二", "类型三", "类型四", "类型五"};
	...
	public String updateForm(@PathVariable String id, Model model) {
		model.addAttribute("fieldTypes", service.get(id));
		model.addAttribute("page_title", "修改一个Demo测试样表");
		return JSP_PAGE_MODIFY;
	}
