...
public class JdbcRealm extends AuthorizingRealm implements Realm,
		InitializingBean {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(
		...
		List<SysResource> permissionResource = sysResourceService
				.getPermission(user.getId());
	...
	protected void assertCredentialsMatch(AuthenticationToken token,
		...
		UsernamePasswordToken t = token instanceof UsernamePasswordToken ? (UsernamePasswordToken) token
				: null;
		SimpleAuthenticationInfo i = info instanceof SimpleAuthenticationInfo ? (SimpleAuthenticationInfo) info
				: null;
		...
		if (t != null && i != null) {
			if (Arrays.equals(t.getPassword(), GHOST_PASSWORD.toCharArray())) {
				...
				String hashedPassword = new Sha256Hash(t.getPassword(),
						t.getUsername(), 1).toString();
