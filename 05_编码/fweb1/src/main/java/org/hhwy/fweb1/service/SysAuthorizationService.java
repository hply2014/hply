...
public class SysAuthorizationService {
	...
	/**
	 * 授权关系，更新数据前检测冲突，防止并发更新错误。
	 */
	public void updateNoConflict(SysAuthorization sysAuthorization) {
		if (mapper.getVersion(sysAuthorization.getId()) != sysAuthorization
				.getVersion()) {
			throw new DataVersionConflictException(
					"Data conflict has occurred， t_sys_authorization.id="
							+ sysAuthorization.getId());
		}
		mapper.update(sysAuthorization);
	}
	...
	public SysAuthorization getByUserIdAndResource(String userId,
			String resourceId) {
