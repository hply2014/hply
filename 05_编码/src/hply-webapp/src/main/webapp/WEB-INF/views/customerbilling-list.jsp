                ...
                <tbody>
                    ...
                    <c:forEach items="${list}" var="customerBilling">
                        <tr>
                            ...
                            <td class="amount"><fmt:formatNumber value="${customerBilling.amount}"
                                    pattern="###,###,###,###,##0.00" /></td>
                           ...
                           <td><c:if test="${customerBilling.status != 1 }">
                                    <shiro:hasPermission name="`customerbilling_check`">
                                        <a class="check" data-confirm-message="客户开票情况：<c:out value="${customerBilling.id}" />，审核后所有数据将不能被修改，是否确认？" href="<s:url value="/customerbilling/check/${customerBilling.id }" />">审核</a>
                                    </shiro:hasPermission>
                 <shiro:hasPermission name="`customerbilling_create`">
