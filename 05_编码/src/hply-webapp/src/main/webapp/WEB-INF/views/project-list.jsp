...
<div class="container main">
	...
	<div class="panel panel-primary">
		<div class="panel-heading">
			...
			共<c:out value="${rowCount}" /> 行，第${pageIndex+1 }页 &nbsp;/&nbsp;共${pageCount }页）
		</div>
				...
				<tbody>
					...
					<c:forEach items="${list}" var="project">
						<tr>
							...
							<td title="<c:out value="${project.createUser}" />"><fmt:formatDate value="${project.trice}"
				...
				<div class="col-sm-8">
					<ul class="pager">
						<c:if test="${pageIndex >= 0 }">
							<li><a href="<s:url value='/project?p=${pageIndex - 1}' />">上一页</a></li>
						</c:if>
						<c:if test="${pageIndex < pageCount -1 }">
							<li><a href="<s:url value='/project?p=${pageIndex + 1}' />">下一页</a></li>
						</c:if>
					</ul>
				</div>
