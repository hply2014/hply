...
<div class="container main">
	<c:if test="${not empty message}">
		...
		<div class="alert alert-success alert-dismissible col-md-offset-2 affix" role="alert">
			...
			<strong><a href="<s:url value="/projectsummary/detail/${projectSummary.id }" />"> ${projectSummary.id }</a></strong>
			，${message}
		</div>
	</c:if>
				...
				<tbody>
					...
					<c:forEach items="${list}" var="projectSummary">
						<tr>
							...
							<td><c:out value="${projectSummary.projectCode}" /><br /> <c:out value="${projectSummary.projectName}" /></td>
							...
							<td class="amount">费率：<fmt:formatNumber value="${projectSummary.managementRate}"
									...
									value="${projectSummary.managementPlanAmount}" pattern="###,###,###,###,##0.00" /> <c:if
							...
							<td class="amount"><c:if test="${projectSummary.partyBillingAmount > 0}">开票：<fmt:formatNumber
									...
									<br />
										...
										value="${projectSummary.partyBillingTotalAmount}" pattern="###,###,###,###,##0.00" />
								</c:if></td>
							<td class="amount"><c:if test="${projectSummary.collectionsAmount > 0}">收款：<fmt:formatNumber
									...
									<br />
										...
										value="${projectSummary.collectionsTotalAmount}" pattern="###,###,###,###,##0.00" />
							...
							<td class="amount"><c:if test="${projectSummary.customerBillingAmount > 0}">开票：<fmt:formatNumber
									...
									<br />
										...
										value="${projectSummary.customerBillingTotalAmount}" pattern="###,###,###,###,##0.00" />
								</c:if></td>
...
<!--
	$(function() {
		$("#searchButton").click(function() {
			self.location = "<s:url value='/projectsummary' />?q=" + $("#searchInput").val();
		});
		...
		for (var i = 0; i < <c:out value="${pageCount}" />; i++) {
			p = p + '<li><a href="<s:url value="/projectsummary"/>?q=${queryText}&p=' + i + '">第' + (i + 1) + '页</a></li>';
		}
