...
<div class="panel panel-default">
  <div class="panel-heading"><strong>收款情况</strong>（<c:out value="${list.size()}" />条数据）</div>
...
<table class="table table-hover">
	...
	<tbody>
		...
		<c:forEach items="${list}" var="collections">
			<tr>
				<td><%=++i%></td>
                <td><span class="glyphicon <c:out value="${collections.status != 1 ? 'glyphicon-file' : ''}" />"></span></td>
				<td><a
				...
				<td><c:if test="${collections.status == 1 }">
								...
								<c:if test="${collections.status != 1 }">
									<shiro:hasPermission name="`collections_create`">
										<a
											href="<s:url value="/collections/modify/{id}"><s:param name="id" value="${collections.id }" /></s:url>">修改</a>
									</shiro:hasPermission>
