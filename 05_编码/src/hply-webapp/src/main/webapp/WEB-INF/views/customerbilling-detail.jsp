...
<shiro:hasPermission name="`customerbilling_modify`">
				<div class="btn-group">
					<a
						href="<s:url value="/customerbilling/modify/{id}"><s:param name="id" value="${customerBilling.id }" /></s:url>"
						class="btn btn-info"><span class="glyphicon glyphicon-pencil"></span>
						修 改 </a>
				</div></shiro:hasPermission>
                <shiro:hasPermission name="`customerbilling_delete`">
				<div class="btn-group">
					<a
						data-confirm-message="客户开票情况数据：<c:out value="${customerBilling.id}" />，将被永久删除，操作不可撤销，是否确认？"
						href="<s:url value="/customerbilling/delete/{id}"><s:param name="id" value="${customerBilling.id }" /></s:url>"
...
<sf:form modelAttribute="customerBilling" cssClass="form-horizontal"
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<p class="form-control-static"><c:out value="${customerBilling.status}" /></p>
		</div>
	</div>
</sf:form>
  </div>
