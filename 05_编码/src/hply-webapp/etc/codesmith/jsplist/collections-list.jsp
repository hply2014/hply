...
<div class="btn-toolbar" role="toolbar">
  <div class="btn-group">
  <a href="<c:url value="/collections/create" />" class="btn btn-info"><span class="glyphicon glyphicon-plus"></span> 新 建 </a>
</div>
</div>
	...
	<tbody>
		...
		<c:forEach items="${list}" var="collections">
			<tr>
				...
				<td><c:out value="${collections.status}" /></td>
				<td><a
					href="<s:url value="/collections/modify/{id}"><s:param name="id" value="${collections.id }" /></s:url>">修改</a>
					| <a class="delete" data-confirm-message="删除后不可恢复，您确认要删除【<c:out value="${collections.id}" />】么？"
