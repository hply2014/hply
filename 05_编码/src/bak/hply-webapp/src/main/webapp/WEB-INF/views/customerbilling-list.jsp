...
<table class="table table-hover">
	...
	<tbody>
		...
		<c:forEach items="${list}" var="customerBilling">
			<tr>
				<td><%=++i%></td>
                <td><span class="glyphicon <c:out value="${customerBilling.status != 1 ? 'glyphicon-file' : ''}" />"></span></td>
				<td><a
				...
				<td><c:if test="${customerbilling.status == 1 }">
								...
								<c:if test="${customerBilling.status != 1 }">
									<shiro:hasPermission name="`customerbilling_create`">
										<a
											href="<s:url value="/customerbilling/modify/{id}"><s:param name="id" value="${customerBilling.id }" /></s:url>">修改</a>
									</shiro:hasPermission>
                    ...
                    <div class="col-sm-8">
                        <ul class="pager">
                            <li class="${pageIndex <= 0 ? 'disabled' :'' }"><a
                                href="<s:url value='/customerbilling?p=${pageIndex - 1}' />">上一页</a></li>
                            <li class="${pageIndex + 1 >= pageCount ? 'disabled' :'' }"><a
                                href="<s:url value='/customerbilling?p=${pageIndex + 1}' />">下一页</a></li>
                        </ul>
                    </div>
